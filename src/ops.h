...
#define CPU_RB(A)     cpu_mem_access (A, 0, 0)
#define CPU_WB(A,X)   cpu_mem_access (A, (X), 1)
...
#define LDrr      OP(LDrr);   \
    cpu.r[r1 + 1] = CPU_RB (cpu.pc);\
    cpu.r[r1] = CPU_RB (cpu.pc + 1);\
    cpu.pc += 2;\

#define LDmSP     OP(LDmSP);  \
    CPU_WW (CPU_RW (cpu.pc), cpu.sp);\
    cpu.pc += 2;\
...
/* Bitwise logic */
#define AND      OP(AND)      cpu.r[A] &= cpu.r[r2];\
    cpu.flags = 0;\
    SET_FLAG_Z (cpu.r[A]);\
    SET_FLAG_H (1);

#define ANHL     OP(ANHL);    cpu.r[A] &= hl;\
    cpu.flags = 0;\
    SET_FLAG_Z (cpu.r[A]);\
    SET_FLAG_H (1);

#define XOR      OP(XOR)      cpu.r[A] ^= cpu.r[r2];\
    cpu.flags = 0;\
    SET_FLAG_Z (cpu.r[A]);\
...
#define OR       OP(OR);      cpu.r[A] |= cpu.r[r2];\
    cpu.flags = 0;\
    SET_FLAG_Z (cpu.r[A]);\

#define ORHL     OP(ORHL)     cpu.r[A] |= hl;\
    cpu.flags = 0;\
    SET_FLAG_Z (cpu.r[A]);\
...
#define DAA      OP(DAA);     {\
		int16_t a = cpu.r[A];\
		if (cpu.f_n) {\
			if (cpu.f_h) a = (a - 0x06) & 0xFF;\
			if (cpu.f_c) a -= 0x60;\
		} else {\
			if (cpu.f_h || (a & 0x0F) > 9) a += 0x06;\
			if (cpu.f_c || a > 0x9F) a += 0x60;\
		}\
		cpu.r[A] = a;\
...
#define RST     OP(RST);   uint16_t n = (opHh - 0x18) << 3; cpu.sp -= 2; CPU_WW (cpu.sp, cpu.pc); cpu.pc = n;
