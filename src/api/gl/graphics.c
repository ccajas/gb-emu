...
"    float dotTint = 0.15;\n"
"    if (color.r + color.g + color.b > 1.1) dotTint = 0.02;\n"
...
"    if (fract(position.x * screenSize.x) >= 0.8) color = mix(color, vec3(1), dotTint * 1.25);\n"
"    if (fract(position.y * screenSize.y) >= 0.8) color = mix(color, vec3(1), dotTint);\n"
"    vec3 offset = texture2D(indexed,\n"
"        vec2(position.x - (0.65 / screenSize.x), position.y - (0.65 / screenSize.y))).rgb;\n"
"    if (offset.r + offset.g + offset.b < 1.1)\n"
"        color *= 0.85;\n"
