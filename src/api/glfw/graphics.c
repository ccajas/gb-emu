...
"    if (fract(position.x * screenSize.x) > 0.6) color = mix(color, vec3(1), dotTint * 1.25);"
"    if (fract(position.y * screenSize.y) > 0.6) color = mix(color, vec3(1), dotTint);"
