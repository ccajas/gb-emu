...
"uniform float rr = 0.700000;\n"
"uniform float rg = 0.680000;\n"
"uniform float rb = 0.720000;\n"
"uniform float colorP = 2.2;\n"
...
"    if (fract(position.x * screenSize.x) > 0.75) color = mix(color, vec3(0), 0.28);"
"    if (fract(position.y * screenSize.y) > 0.75) color = mix(color, vec3(0), 0.15);"
"    color *= 1.1;\n"
"    return color;\n"
"}\n"
...
"{\n"
"    vec2 position = (TexCoords.xy);\n"
"    if (fract(position.x * screenSize.x) > 0.75) color = mix(color, vec3(0), 0.15);\n"
"    float tr = sin(position.x * screenSize.x * PI_2) + color.r;\n"
"    float tg = sin((position.x + 0.33) * screenSize.x * PI_2) + color.g;\n"
"    float tb = sin((position.x + 0.67) * screenSize.x * PI_2) + color.b;\n"
"    color.r *= tr * rr; color.g *= tg * rg; color.b *= tb * rb;\n"
"    color.r = pow(color.r, colorP);\n"
"    color.g = pow(color.g, colorP);\n"
"    color.b = pow(color.b, colorP);\n"
"    if (fract(position.y * screenSize.y) > 0.75) color = mix(color, vec3(0), 0.4);\n"
